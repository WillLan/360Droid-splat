...
def _aligned_initializer() -> PanoCompactGaussianInitializer:
...
def test_panovggt_aligned_initializer_outputs_dense_sh3_state_and_masks_sky():
...
def test_panovggt_aligned_offset_updates_means_in_spherical_frame():
    images, features, depths, poses, valid, _sky, world = _aligned_batch()
    model = _aligned_initializer()
    state_zero = model(images, features, depths, poses, valid, world_points=world)

    with torch.no_grad():
        model.offset_proj.bias[0] = 5.0
...
def test_panovggt_aligned_soft_splat_forward_loss_backward():
    images, features, depths, poses, valid, sky, world = _aligned_batch(h=5, w=10)
    sample = {"images": images, "valid_depth": valid, "sky_mask": sky}
    ...
    cfg["Initializer"].update({"position_mode": "panovggt_aligned", "state_dim": 8, "sh_degree": 3, "max_gaussians": 0, "gaussians_per_cell": 1})
