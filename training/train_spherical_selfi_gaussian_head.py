...
def train_spherical_selfi_gaussian_head(config: dict[str, Any]) -> dict[str, Any]:
    ...
    _seed_everything(int(train_cfg.get("seed", 1234)))
    ...
    head = build_head(config, device=train_device)
    renderer = build_renderer(config)
    lpips_model = _optional_lpips(config, train_device)
    ...
    def make_observation(raw_images: torch.Tensor, frame_ids: torch.Tensor) -> tuple[PerPixelGaussianObservation, torch.Tensor]:
        ...
        return predict_observation(dense, rgb, depth, poses, frame_ids), rgb
    ...
    while step < max_steps:
        ...
        for batch in loader:
            ...
            divisor = float(int(rgb.shape[0]) * int(rgb.shape[1]))
            ...
            for item in range(int(rgb.shape[0])):
                for target_view in range(int(rgb.shape[1])):
                    ...
                    with torch.amp.autocast(
                        ...
                        if first_package is None:
                            first_package = {
                                ...
                                "profile_renderer_total_sec": package.get("profile_renderer_total_sec", 0.0),
                        ...
                        if float(weights.geometry) != 0.0:
                            ...
                            geometry = spherical_pseudo_geometry_consistency_loss(
                                ...
                                visibility_rel_thresh=float(loss_cfg.get("geometry_visibility_rel_thresh", 0.05)),
                            )
                    ...
                    for key, value in terms.items():
                        aggregate[key] = aggregate.get(key, torch.zeros_like(value.detach())) + value.detach() / divisor
            ...
            scheduler.step()
            step += 1
            latest_metrics = {key: float(value.detach().float().cpu()) for key, value in aggregate.items()}
            latest_metrics["loss"] = total_loss_value
            if first_package:
                for key in (
                    ...
                    latest_metrics[key] = float(first_package.get(key, 0.0))
            latest_metrics.update(_observation_diagnostics(last_observation, batch_index=0))
            latest_metrics["grad_norm"] = float(grad_norm.detach().float().cpu())
            latest_metrics["lr"] = float(optimizer.param_groups[0]["lr"])
            latest_metrics["elapsed_sec"] = float(time.perf_counter() - start_time)
            if train_device.type == "cuda":
                latest_metrics["peak_memory_gib"] = float(torch.cuda.max_memory_allocated(train_device) / (1024**3))
            if step == 1 or step % int(train_cfg.get("log_interval", 50)) == 0:
                print(yaml.safe_dump({"step": step, "metrics": latest_metrics}, sort_keys=False).strip())
            if wandb_run is not None and (step == 1 or step % int(config["WeightsAndBiases"].get("log_every", 50)) == 0):
                wandb_run.log({f"train/{key}": value for key, value in latest_metrics.items()}, step=step)
            if (
                bool(vis_cfg.get("enabled", False))
            ...
            if val_loader is not None and (step % int(train_cfg.get("val_interval", 1000)) == 0 or step == max_steps):
            ...
            if step % int(train_cfg.get("save_interval", 1000)) == 0 or step == max_steps:
                save_stage2_checkpoint(
