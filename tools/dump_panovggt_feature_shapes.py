    ...
    *,
    ...
    if manifest is None:
        ...
        dataset = Stage1PanoSequenceDataset(
            manifest,
            split=str(dataset_cfg.get("shape_dump_split", "train")),
            domains=list(dataset_cfg.get("domains", ["indoor", "outdoor"])),
            views_per_sample=num_views,
            image_height=height,
            image_width=width,
            max_temporal_gap=dataset_cfg.get("max_temporal_gap", 10),
        )
        loader = DataLoader(
            dataset,
        ...
        images = batch["images"].float().to(torch_device)
    with torch.no_grad():
        output = wrapper(images)
        dense = adapter(output.stage_features)
    pano_params = [param for param in wrapper.model.parameters()]
    adapter_params = [param for param in adapter.parameters()]
    return {
        ...
        "panovggt_requires_grad": any(param.requires_grad for param in pano_params),
        "adapter_requires_grad": any(param.requires_grad for param in adapter_params),
