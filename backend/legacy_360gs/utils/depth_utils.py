    ...
    for k in range(max_iter):
        ...
        if (np.sum(accurate_pixels) < min_accurate_pixels) and (k==2):
            ...
            if model is not None:
                ...
                # No MASt3R model available (DAP+SphereGlue mode): use global
                # median depth ratio as a lightweight fallback.
                valid = (render_depth > 0) & (mono_depth > 0)
                if valid.any():
                    scale_factor = float(np.median(render_depth[valid] / mono_depth[valid]))
            continue
        ...
        if (np.sum(accurate_pixels) < min_accurate_pixels) and (k==3):
            ...
            if model is not None:
                ...
                if valid.any():
                    scale_factor = float(np.median(render_depth[valid] / mono_depth[valid]))
                print("Fallback: not enough accurate pixels, using global median depth ratio (no MASt3R)")
            break
