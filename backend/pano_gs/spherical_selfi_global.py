...
class SphericalSelfiGlobalBackend:
    ...
    def _run_map_optimization(self, window_id: int, frame_ids: tuple[int, ...], steps: int) -> dict[str, float]:
        ...
        try:
            ...
            pose_refine_enabled = (
                bool(self.map_optimize_config.get("pose_refine_enable", False))
                if self.boundary_frame_graph
                else True
            )
            fixed_frame_ids = (
                []
                if self.boundary_frame_graph and pose_refine_enabled
                else [int(self.packets[self.window_order[0]].frame_ids[0])]
            ...
            settings = {
                ...
                "pose_lr": (
                    float(self.map_optimize_config.get("pose_lr", 1.0e-3))
                    if pose_refine_enabled
                    else 0.0
                ),
                "pose_refine_enable": pose_refine_enabled,
            ...
            metrics["pose_refine_enabled"] = float(pose_refine_enabled)
    ...
    def _joint_graph_pose_loss(self, window_id: int, trainable_pose_ids: set[int]) -> torch.Tensor:
        ...
        for factor in self.graph.edges:
            if not isinstance(factor, DenseSphericalFactorBlock):
                continue
            if (
                not self.boundary_frame_graph
                and int(factor.source) != int(window_id)
                and int(factor.target) != int(window_id)
            ):
                continue
            ...
            if source_frame_id is None or target_frame_id is None:
                continue
            if self.boundary_frame_graph and (
    ...
    def _synchronize_boundary_optimized_window(self, window_id: int) -> None:
        """Synchronize render-optimized SE(3) poses with boundary-frame graph nodes."""
        ...
        try:
            # Sim(3) scale stays graph-owned; render refinement contributes only
            # the optimized global SE(3) rotation and translation.
            for frame_id, global_pose in optimized_by_frame.items():
            ...
            self._refresh_factor_local_poses(affected_windows)
            # Keep global graph BA on its configured cadence.
            self._refresh_geometry_updates()
        except Exception:
            self.graph.nodes = {node: value.clone() for node, value in old_nodes.items()}
            self._geometry_updates = geometry_snapshot
